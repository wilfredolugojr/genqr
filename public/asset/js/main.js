(function (jQuery) {
    ...
    $("#campo").keydown(function (e) {
        ...
        if ((e.shiftKey || (e.keyCode < 48 || e.keyCode > 57)) && (e.keyCode < 96 || e.keyCode > 105) || $(this).val().length >= 11 ) {
            e.preventDefault();
        }
    });

    $('#genqr').click(function(){
        if($("#validate_email").val().indexOf('@', 0) == -1 || $("#validate_email").val().indexOf('.', 0) == -1) {
            $('#validate_email-error').show();
